...
| Hosts | Local and share hosts by copy; WSL through the `\\wsl$` share; SSH hosts through one `ssh <host> tar czf - -T -` stream after a size/mtime check, so unchanged files are not transferred. |
